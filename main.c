...
void delete_head_list()
{
    ...
    if(head_list == NULL)
    {
        return;                      // список пуст-вернули обещанное значение
    }// if(head_list == NULL)
...
void Ctrl_plus_C(int sig)            // сигнал, который нужно перехватить
{
    ...
    else
    {
        ...
        if(head_list != NULL)        // в списке дочерних процессов есть хотя бы один элемент
        {
...
void create_new_process(char **con_args) // указатель на многомерный массив строки консоли, разделённый на отдельные аргументы
{
    ...
    pid_t waiting;                   // ожидание завершения дочернего процесса   
    int status;                      // хранение статуса выполнения дочернего процесса
    ...
    switch(pid)
    {
        ...
        case 0: 
            ...
            if(success == -1)        // проверяет, вернул ли execvp() ошибку. Если вернул, значит команда не найдена
            {
                ...
                // прекратить выполнение процесса в случае ошибки досрочно - переводим в состояние "зомби"
                kill(getpid(), SIGTERM);
            }// if(success == -1)
        break;
        ...
        default:
            // с waitpid для завершения отработанных  дочерних-процессов "зомби"
            // проверяется, что дочерний процесс не завершился нормально
            // блокирует родительский процесс и ожидает завершения одного из дочерних процессов - возвращает идентификатор завершившегося дочернего процесса
            while (!WIFEXITED(status) && (waiting = wait(&status)) != -1) 
            {
                printf("\033[38;5;135mДочерний процесс\033[0m = \033[38;5;228m%d\033[0m\n", pid);
            }// while()

            // добавляемый дочерний процесс в список - файтически он уже завершился
            pid_t new_pid = -1;

            // есть доступные дочерние процессы и завершился доступный дочерний процесс
            if(waiting != 0 && waiting != -1)
            {
                // присваиваем PID завершенного процесса
                new_pid = waiting;
            }
            ...
            {
                // присваивается значение текущего идентификатора процесса
                new_pid = getpid();
            }// if(waiting != 0 && waiting != -1)
